<template>
  <div class="space-y-8">
      ...
      <div class="flex flex-col md:flex-row gap-4">
        <div class="flex-1 relative">
          ...
          <input 
            ...
            class="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-600/10 focus:border-blue-600 outline-none transition-all font-medium text-sm"
          >
        </div>
        <div class="flex gap-4">
          <select v-model="statusFilter" class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-medium text-sm text-gray-600 focus:border-blue-600">
            <option value="">All statuses</option>
            <option v-for="status in agentStatuses" :key="status" :value="status">{{ status }}</option>
          </select>
          <select v-model="tierFilter" class="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-medium text-sm text-gray-600 focus:border-blue-600">
    ...
    <!-- Agents Table -->
    <UiBaseCard class="!p-0 overflow-hidden !rounded-none">
      <UiBaseTable 
        :columns="columns" 
        :items="agents" 
        :meta="meta"
      ...
      >
        <template #cell(agency)="{ item }">
          <div class="flex flex-col">
            <span class="font-semibold text-gray-900">{{ item.tenant?.name || 'Independent' }}</span>
          </div>
        </template>
        ...
        <template #cell(phone)="{ item }">
          <span class="text-sm text-gray-600">{{ item.phone || 'N/A' }}</span>
        </template>
        
        <template #cell(walletBalance)="{ item }">
          <span class="text-sm font-semibold text-gray-900">{{ item.preferences?.currency || 'NGN' }} {{ item.walletBalance?.toLocaleString() || '0' }}</span>
        </template>

        <template #cell(status)="{ item }">
          <span class="px-3 py-1 rounded-full text-sm font-bold uppercase " :class="getStatusBadgeClass(item.agentStatus)">
            {{ item.agentStatus || 'N/A' }}
          </span>
        </template>
        ...
        <template #cell(actions)="{ item }">
          <div class="flex justify-end pr-2">
            <NuxtLink :to="`/dashboard/agents/${item._id}`" class="h-8 w-8 flex items-center justify-center bg-gray-50 hover:bg-blue-600 hover:text-white text-gray-400 rounded-lg transition-all">

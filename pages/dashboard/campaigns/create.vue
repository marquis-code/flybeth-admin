<template>
  <div class="space-y-10 max-w-7xl mx-auto pb-20">
    <!-- Action Header -->
    <div class="flex justify-between items-center bg-white border border-gray-200 p-8 rounded-3xl transition-premium shadow-none">
      <div class="flex items-center space-x-6">
        <button @click="navigateTo('/dashboard/campaigns')" class="h-14 w-14 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 hover:bg-brand-blue hover:text-white transition-premium group shadow-none">
      ...
      <div class="flex gap-4">
        <UiBaseButton variant="secondary" @click="showPreviewModal = true" class="h-14 font-bold">
           <EyeIcon class="h-5 w-5 mr-2" />
           Preview
        </UiBaseButton>
        <UiBaseButton variant="secondary" @click="navigateTo('/dashboard/campaigns')" class="h-14 font-bold">Cancel</UiBaseButton>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
      ...
      <div class="lg:col-span-2 space-y-10">
        ...
        <div class="bg-white border border-gray-200 rounded-[2.5rem] p-12 space-y-12 shadow-none">
           ...
           <div class="space-y-10">
              <div class="space-y-3">
                 ...
                 <input 
                   ...
                   class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-6 px-8 text-lg font-bold text-gray-900 focus:ring-4 focus:ring-brand-blue/5 focus:bg-white transition-premium outline-none shadow-none"
                 />
              </div>
              <div class="space-y-3">
              ...
              <div class="space-y-3">
                 ...
                 <div 
                   @click="triggerUpload"
                   class="relative group cursor-pointer overflow-hidden rounded-[2.5rem] border-2 border-dashed border-gray-200 hover:border-brand-blue/30 transition-premium aspect-[21/9] flex items-center justify-center bg-gray-50 shadow-none"
                 >
                    ...
                    <div v-if="uploading" class="absolute inset-0 bg-white/95 flex flex-col items-center justify-center z-50 animate-in fade-in duration-300">
                       <div class="relative">
                          <div class="h-16 w-16 border-4 border-gray-200 border-t-brand-blue rounded-full animate-spin"></div>
                    ...
                    <div v-if="form.imageUrl && !uploading" class="absolute top-6 right-6 z-20">
                       <button @click.stop="form.imageUrl = ''" class="p-3 bg-white/90 backdrop-blur-sm hover:bg-red-500 hover:text-white rounded-2xl transition-premium shadow-none border border-gray-200">
      ...
      <div class="space-y-10">
        ...
        <div class="bg-white border border-gray-200 rounded-[2.5rem] p-10 space-y-10 sticky top-24 shadow-none">
           <div>
              ...
              <div class="relative z-50">
                 ...
                 <div v-if="showAudienceList" @click="showAudienceList = false" class="fixed inset-0 z-40"></div>
                 <button 
                   @click="showAudienceList = !showAudienceList"
                   class="w-full flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-200 hover:border-brand-blue hover:bg-white transition-premium outline-none shadow-none relative z-50"
                 >
                    <div class="flex items-center space-x-4 text-left">
                       <div class="h-10 w-10 bg-white rounded-xl flex items-center justify-center text-brand-blue shadow-none border border-gray-200">
                          <component :is="selectedAudience?.icon || BoltIcon" class="h-5 w-5" />
                       </div>
                       <div>
                          <p class="text-sm font-bold text-gray-900">{{ selectedAudience?.label }}</p>
                          <p class="text-[11px] text-gray-400 font-medium">{{ selectedAudience?.userFriendly }}</p>
                       </div>
                    </div>
                    <ChevronDownIcon class="h-5 w-5 text-gray-400 transition-premium" :class="{ 'rotate-180': showAudienceList }" />
                 </button>

                 <!-- Dropdown List -->
                 <div v-if="showAudienceList" class="absolute z-[60] w-full mt-2 bg-white border border-gray-200 rounded-2xl p-2 animate-in fade-in zoom-in-95 duration-200 shadow-none border-b-4 border-b-brand-blue/5">
                    <button 
                      v-for="aud in audiences" 
                      :key="aud.value"
                      @click="selectAudience(aud)"
                      class="w-full flex items-center p-3 rounded-xl hover:bg-gray-50 transition-premium group text-left"
                      :class="{ 'bg-brand-blue/5': form.targetAudience === aud.value }"
                    >
                       <div class="h-10 w-10 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 group-hover:bg-brand-blue group-hover:text-white transition-premium mr-3 shadow-none">
                          <component :is="aud.icon" class="h-5 w-5" />
                       </div>
                       <div>
                          <p class="text-sm font-bold text-gray-900">{{ aud.label }}</p>
                          <p class="text-[11px] text-gray-400 font-medium">{{ aud.description }}</p>
                       </div>
                    </button>
                 </div>
              </div>
           ...
           <div v-if="form.targetAudience === 'roles'" class="animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div class="flex flex-wrap gap-3">
                 <button 
                  ...
                  :class="form.targetRoles.includes(role.value) ? 'bg-brand-blue border-brand-blue text-white' : 'bg-gray-50 border-gray-200 text-gray-500 hover:border-brand-blue/30'"
                 >
                   {{ role.label }}
                 </button>
              </div>
           ...
           <!-- Airline Input -->
           <div v-if="form.targetAudience === 'airline'" class="animate-in fade-in slide-in-from-top-2 duration-300 space-y-3">
              <p class="text-xs font-bold text-gray-400 uppercase">Enter Airline</p>
              <input 
                v-model="form.targetAirline" 
                type="text" 
                placeholder="e.g. Emirates"
                class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-4 px-6 text-sm font-bold text-gray-900 outline-none focus:bg-white focus:ring-4 focus:ring-brand-blue/5 transition-premium shadow-none"
              />
           </div>

           <!-- Booking Input -->
           <div v-if="form.targetAudience === 'booking'" class="animate-in fade-in slide-in-from-top-2 duration-300 space-y-3">
              <p class="text-xs font-bold text-gray-400 uppercase">Enter Booking Reference (PNR)</p>
              <input 
                v-model="form.targetBookingReference" 
                type="text" 
                placeholder="e.g. A3B4C5"
                class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-4 px-6 text-sm font-bold text-gray-900 outline-none focus:bg-white focus:ring-4 focus:ring-brand-blue/5 transition-premium shadow-none"
              />
           </div>
           ...
           <div v-if="form.targetAudience === 'specific'" class="animate-in fade-in slide-in-from-top-2 duration-300 space-y-5">
              ...
              <div class="relative">
                 <input 
                   ...
                   class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-4 px-6 text-sm font-bold text-gray-900 outline-none focus:bg-white focus:ring-4 focus:ring-brand-blue/5 transition-premium shadow-none"
                 />
                 <div v-if="userSearchResults.length > 0" class="absolute z-50 w-full mt-3 bg-white border border-gray-200 rounded-[1.5rem] p-2 shadow-none border-b-4 border-b-brand-blue/5">
              ...
              <div class="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-2">
                 <div v-for="user in selectedUsersDetails" :key="user._id" class="flex items-center px-4 py-2.5 bg-gray-50 rounded-xl border border-gray-200 animate-in fade-in zoom-in-50 shadow-none">
           ...
           <div class="relative z-40">
              ...
              <div class="relative">
                 <!-- Click-outside Overlay -->
                 <div v-if="showTimingList" @click="showTimingList = false" class="fixed inset-0 z-40"></div>
                 <button 
                   @click="showTimingList = !showTimingList"
                   class="w-full flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-200 hover:border-brand-blue hover:bg-white transition-premium outline-none shadow-none relative z-50"
                 >
                    <div class="text-left">
                       <p class="text-sm font-bold text-gray-900">{{ selectedTiming?.label }}</p>
                       <p class="text-[11px] text-gray-400 font-medium">{{ selectedTiming?.description }}</p>
                    </div>
                    <ChevronDownIcon class="h-5 w-5 text-gray-400 transition-premium" :class="{ 'rotate-180': showTimingList }" />
                 </button>

                 <!-- Timing Dropdown -->
                 <div v-if="showTimingList" class="absolute z-[60] w-full mt-2 bg-white border border-gray-200 rounded-2xl p-2 animate-in fade-in zoom-in-95 duration-200 shadow-none border-b-4 border-b-brand-blue/5">
                    <button 
                      v-for="time in timingOptions" 
                      :key="time.value"
                      @click="selectTiming(time)"
                      class="w-full p-3 rounded-xl hover:bg-gray-50 transition-premium group text-left shadow-none"
                      :class="{ 'bg-brand-blue/5': sendTiming === time.value }"
                    >
                       <p class="text-sm font-bold text-gray-900">{{ time.label }}</p>
                       <p class="text-[11px] text-gray-400 font-medium group-hover:text-brand-blue transition-premium">{{ time.description }}</p>
                    </button>
                 </div>
              </div>
              
              <!-- Scheduling UI -->
              <div v-if="sendTiming === 'schedule'" class="mt-5 p-5 bg-gray-50 rounded-[2rem] border border-gray-200 animate-in fade-in zoom-in-95 duration-300 shadow-none space-y-4">
                 <UiCustomDatePicker 
           ...
           <!-- Summary Section -->
           <div class="p-8 bg-gray-50 rounded-[2rem] space-y-5 border border-gray-200 shadow-none">
              ...
              <div class="h-1.5 w-full bg-white rounded-full overflow-hidden border border-gray-200">
    ...
    <!-- Preview Modal -->
    <UiBaseModal :show="showPreviewModal" @close="showPreviewModal = false" title="Campaign Preview" maxWidth="max-w-5xl">
...
const audiences = [
  ...
  { label: 'Airline customers', icon: PaperAirplaneIcon, value: 'airline', userFriendly: 'Target by airline', description: 'Users with bookings for a specific airline.' },
  { label: 'Specific booking', icon: TicketIcon, value: 'booking', userFriendly: 'Target a single booking', description: 'Send to passengers of a specific PNR.' },
...
const recipientCount = computed(() => {
   ...
   if (form.value.targetAudience === 'airline') return 'Dynamic (Airline)'
   if (form.value.targetAudience === 'booking') return 'Dynamic (Booking)'
   return 'Segmented'
})

const previewContent = computed(() => {
   if (!form.value.content && !form.value.imageUrl) return ''
   let html = '<div style="font-family: \'Inter\', sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b; padding: 20px;">'
   if (form.value.imageUrl) {
      html += `<img src="${form.value.imageUrl}" style="width: 100%; max-height: 300px; object-fit: cover; border-radius: 12px; margin-bottom: 24px;" alt="Campaign Banner" />`
   }
   html += `<div style="line-height: 1.6;">${form.value.content || ''}</div>`
   html += '</div>'
   return html
})
...
const saveCampaign = async () => {
  ...
  if (sendTiming.value === 'schedule' && (!form.value.scheduleDate || !form.value.scheduleTime)) {
    showToast({ title: 'Logic Error', message: 'Please select a date and time for the scheduled send.', toastType: 'error' })
    return
  }
  ...
  try {
    ...
    if (sendTiming.value === 'schedule') {
        const timeMap: Record<string, string> = { 'Morning': '09:00', 'Noon': '12:00', 'Evening': '18:00', 'Midnight': '00:00' }
        const timeStr = timeMap[form.value.scheduleTime] || form.value.scheduleTime
        const [hours, minutes] = timeStr.split(':')
        
        const d = new Date(form.value.scheduleDate as Date)
   ...
   if (isEdit.value) {
      ...
      if (existing) {
         ...
         if (existing.scheduledAt) {
            ...
            form.value.scheduleDate = d
            form.value.scheduleTime = `${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}`
